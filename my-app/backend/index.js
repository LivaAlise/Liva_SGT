...
app.get("/get-allCustomer", (req, res) => {
    ...
    connection.query(sqlQuery, (error, results) => {
        if (error)
            throw error;
        res.send(JSON.stringify({ "status": 200, "error": null, "response": { customer: results } }))
    })
})
...
app.post("/add-customer", (req, res) => {
    ...
    const sqlQuery = "INSERT INTO customer (firstname,lastname,email,phone,vip) VALUES (?,?,?,?,?)";
...
app.post ("/delete-customer", (req, res) => {
    ...
    connection.query(sqlQuery, [req.body.delete], (error, results) => {
        if (error)
            throw error;
        res.send(JSON.stringify({ "status": 200, "error": null, "response": { affected: results.affectedRows } 
}))})})
